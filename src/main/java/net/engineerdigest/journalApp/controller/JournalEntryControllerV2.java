...
import net.engineerdigest.journalApp.service.JournalEntryService;
import net.engineerdigest.journalApp.service.UserService;
...
public class JournalEntryControllerV2 {
	...
	@PutMapping("id/{myId}")
	public ResponseEntity<?> updateJournalByIdEntry(@PathVariable ObjectId myId, @RequestBody JournalEntry newEntry) {
		Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
		String userName = authentication.getName();
		User user = userService.findByUserName(userName);
		List<JournalEntry> collect = user.getJournalEntry().stream().filter(x ->x.getId().equals(myId)).collect(Collectors.toList());
